...
function review_exam_content($atts) {
    ...
    // Get post_ids from imported_questions table based on question_ids from exam_answers
    $question_ids = $wpdb->get_col($wpdb->prepare(
        "SELECT DISTINCT iq.post_id 
         FROM $answers_table ea
         INNER JOIN $questions_table iq ON ea.question_id = iq.id
         WHERE ea.user_id = %d AND ea.question_category = %d 
         ORDER BY iq.post_id",
        $user_id, $category_l
    ));
...
function display_flashcard_solution($user_id, $exam_id, $selected_questions, $exam_title, $class_id, $type = 'exam') {
    ...
    if (empty($selected_questions)) {
        return '<div class="no-questions-message" style="text-align: center; padding: 40px; background: #f8f9fa; border-radius: 10px; margin: 20px 0;">
                    ...
                    <p style="color: #6c757d; margin-bottom: 10px;">There are currently no questions available for review.</p>
                    <p style="color: #6c757d;">Please check back later or contact your instructor.</p>
                    <a href="' . home_url('/profile?class_id=' . $class_id . ($cat ? '&cat=' . $cat : '')) . '" class="button-primary" style="margin-top: 20px; display: inline-block; border-radius: 10px; padding: 15px 30px;">← Go Back</a>
                </div>';
    }
    ...
    <div class="flashcard-solution-container">
        ...
        <h5><?php echo esc_html($exam_title); ?></h5>
    ...
    jQuery(document).ready(function($) {
        ...
        loadQuestionBatch(0, function(success) {
            ...
            if (success && loadedQuestions.length > 0) {
                ...
                container.html('<p style="text-align: center; padding: 20px;">No questions available.</p>');
            }
        });
    });
    </script>
...
function load_flashcard_questions() {
    ...
    // Fetch questions from custom table using post_id
    $questions = $wpdb->get_results(
        $wpdb->prepare(
            "SELECT id, post_id, title, category, option_a, option_b, option_c, option_d, 
             ...
             ORDER BY FIELD(post_id, $placeholders)",
            array_merge($question_ids, $question_ids) // Need to pass question_ids twice for both IN and FIELD
        ),
        ARRAY_A
    );
    
    // Debug: Log what was requested vs what was found
    error_log('Requested question IDs: ' . print_r($question_ids, true));
    error_log('Found questions: ' . count($questions));
    
    if (empty($questions)) {
        // Additional debugging query to see what's actually in the table
        $all_post_ids = $wpdb->get_col("SELECT post_id FROM $questions_table LIMIT 10");
        error_log('Sample post_ids in table: ' . print_r($all_post_ids, true));
        
        wp_send_json_error('No questions found. Requested IDs: ' . implode(',', $question_ids));
    }
    ...
    foreach ($questions as $question) {
        ...
        $question_data[] = array(
            'id' => $question['post_id'], // Use post_id for compatibility
            'title' => esc_html($question['title']),
            'options' => $multiple_choice_options,
            'correct_answer' => $question['correct_answer'],
            'image_url' => !empty($question['image_url']) ? esc_url($question['image_url']) : '',
            'solution' => !empty($question['solution']) ? nl2br(esc_html($question['solution'])) : ''
        );
    }
